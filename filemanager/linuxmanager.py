import os
"""
Код линуксовой части
Как и для винды здесь есть 6 функций: открыть файл/папку, записать в файл, создать файл/папку, удалить(тут только файл
удалять папку в задании не просят), переименовать

Все работает достаточно просто: на выбор предлагают команду, дальше идет вывод всех файлов списком в терминал.

Список генерится функцией(она написана ниже), дальше пользователь вводит путь до файла, если он корректен, то происходит
...
Все возможные ошибки типа файл не найден уже прописаны внутри

И еще важная деталь: то, что эта вещь может работать бесконечно, пока пользователь не даст команду на выход, обеспечено
тем, что при каждом выходе из опции просто происходит прерывание цикла while, потому что еслделать sys.exit(0) то 
программа просто вырубится
"""
